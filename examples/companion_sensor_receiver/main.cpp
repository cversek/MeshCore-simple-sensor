...
class ReceiverMesh : public BaseChatMesh, ContactVisitor {
  ...
  bool postToBayou(float temp, float batt, const char* from_name, uint16_t nid) {
    ...
    snprintf(body, sizeof(body),
             "{\"private_key\":\"%s\",\"node_id\":%u,\"temperature_c\":%.2f,\"battery_volts\":%.3f,\"source\":\"%s\"}",
             _prefs.bayou_private_key, (unsigned)nid, (double)temp, (double)batt, from_name);
...
protected:
  ...
  void onMessageRecv(const ContactInfo& from, mesh::Packet* pkt, uint32_t sender_timestamp, const char *text) override {
    ...
    if (parseSensorMessage(text, temp, batt, nid)) {
      handleSensorData(from.name, temp, batt, nid);
    }
  }
...
public:
  ...
  void handleCommand(const char* command) {
    ...
    if (memcmp(command, "list", 4) == 0) {
      ...
      Serial.println("   list {n}                      - list contacts");
