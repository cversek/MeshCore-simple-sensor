...
#define FIRMWARE_VER_TEXT   "companion_sensor v3 (build: May 2026) [v2 + OLED ACK status on long-press send]"
...
static void renderStatusPage(DisplayDriver& d) {
  ...
  switch (the_mesh.getLastAckStatus()) {
    case ACK_STATUS_OK: {
      // Time since the last successful ACK, with auto-scaling units so the
      // display stays readable on long-running nodes.
