...
### Step 4: Set the Target Contact

Tell the sensor which contact should receive the readings (sensor serial monitor):

```
target ReceiverName
```

This does a name prefix match, so `target Rec` works too. Verify with:
...
[SENSOR] node_id=1 temp=22.50C batt=3.85V
   Sending to ReceiverName: [SENSOR] node_id=1 temp=22.50C batt=3.85V
   (message sent - FLOOD)
...
| `list {n}` | List contacts (optionally last n) |
| `target <name>` | Set send target by name prefix |
