...
#define FIRMWARE_VER_TEXT   "companion_sensor_receiver_route v1 (build: Apr 2026)"
...
class ReceiverMesh : public BaseChatMesh, ContactVisitor {
  ...
  char last_recv_path_hex[130];  // up to 64 bytes * 2 hex chars + null
  bool has_recv;
  ...
  bool postToBayou(float temp, float batt, const char* from_name, uint16_t nid, uint8_t hops, bool direct, const char* path_hex) {
    ...
    snprintf(body, sizeof(body),
             "{\"private_key\":\"%s\",\"node_id\":%u,\"temperature_c\":%.2f,\"battery_volts\":%.3f,\"aux_1\":%u,\"aux_2\":%u,\"log\":\"%s\",\"source\":\"%s\"}",
             _prefs.bayou_private_key, (unsigned)nid, (double)temp, (double)batt,
             (unsigned)hops, direct ? 1u : 0u, path_hex ? path_hex : "", from_name);
  ...
  void handleSensorData(const char* from_name, float temp, float batt, uint16_t nid, uint8_t hops, bool direct, const char* path_hex) {
    last_recv_temp = temp;
    last_recv_batt = batt;
    ...
    StrHelper::strncpy(last_recv_path_hex, path_hex ? path_hex : "", sizeof(last_recv_path_hex));
    has_recv = true;

    Serial.printf("   [SENSOR DATA] from=%s node_id=%u temp=%.2fC batt=%.2fV radio_hops=%u route=%s path=%s\n",
                  from_name, (unsigned)nid, temp, batt, (unsigned)hops,
                  direct ? "DIRECT" : "FLOOD", last_recv_path_hex);
...
protected:
  ...
  void onMessageRecv(const ContactInfo& from, mesh::Packet* pkt, uint32_t sender_timestamp, const char *text) override {
    ...
    if (parseSensorMessage(text, temp, batt, nid)) {
      // Hop count + path bytes:
      //  FLOOD: pkt->path[] accumulates in transit, count == hops.
      //  DIRECT: path is stripped by repeaters, count always 0 at destination.
      uint8_t hops = pkt->getPathHashCount();
      char path_hex[130] = {0};
      if (pkt->isRouteFlood() && hops > 0) {
        uint8_t n = hops * pkt->getPathHashSize();
        if (n > 64) n = 64;
        for (uint8_t k = 0; k < n; k++) {
          snprintf(&path_hex[k*2], 3, "%02x", pkt->path[k]);
        }
      }
      handleSensorData(from.name, temp, batt, nid, hops, pkt->isRouteDirect(), path_hex);
    }
  }
...
public:
     ...
     : BaseChatMesh(radio, *new ArduinoMillis(), rng, rtc, *new StaticPoolPacketManager(16), tables)
  {
    ...
    memset(last_recv_path_hex, 0, sizeof(last_recv_path_hex));
    last_recv_temp = 0;
    last_recv_batt = 0;
    last_recv_node_id = 0;
    last_recv_hops = 0;
  ...
  void handleCommand(const char* command) {
    ...
    if (memcmp(command, "list", 4) == 0) {
      ...
      if (has_recv) {
        Serial.printf("   Last: from=%s node_id=%u temp=%.2fC batt=%.2fV radio_hops=%u route=%s path=%s\n",
  ...
  void serviceBayou() {
    ...
    if (postToBayou(last_recv_temp, last_recv_batt, last_recv_from, last_recv_node_id, last_recv_hops, last_recv_direct, last_recv_path_hex)) {
      post_pending = false;
    } else {
